...
async def search_videos_with_metadata(
    query: str, fields: list[str] = None
) -> list[dict]:
    """For possible fields, see the metadata method in src/salon/anothertube/video_data.py"""
    ids = await search_videos_ids(query)
    data = await asyncio.gather(
        *(_delayed_video_data(vid, delay_ms=i * 100) for i, vid in enumerate(ids))
    )

    if fields:
        data = [
            {field: vid[field] for field in fields} for vid in data if vid is not None
        ]

    if all((len(d) == 0 for d in data)):
        raise ValueError("No result at all, might have been rate limited.")
